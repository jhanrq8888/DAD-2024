...
import { Proveedor } from '../../models/proveedor'; // Modelo actualizado
...
export class ProveedorEditComponent implements OnInit {
    ...
    @Input() title: string = '';
    @Input() proveedor = new Proveedor(); // Modelo actualizado
    abcForms: any;

    constructor(
        private formBuilder: FormBuilder,
        private _matDialog: MatDialogRef<ProveedorEditComponent>,
    ) {}

    ngOnInit() {
        this.abcForms = abcForms;

        // Si hay datos del proveedor, rellenar el formulario
        if (this.proveedor) {
    ...
    public saveForm(): void {
        if (this.proveedorForm.valid) {
            this._matDialog.close(this.proveedorForm.value); // Enviar los datos actualizados
        }
    }

    public cancelForm(): void {
        this._matDialog.close(''); // Cancelar edición
    }
}
